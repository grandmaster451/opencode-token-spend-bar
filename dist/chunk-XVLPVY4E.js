...
function SessionSpendWidget(props) {
  ...
  for (const row of props.viewModel.rows) {
    ...
    insert3(text, () => {
      ...
      let displayFormatted = animatedTokens;
      if (row.hasRemainingQuota && row.remaining !== null) {
        displayFormatted += `  (${row.remaining}, ${row.percentage}%)`;
      }
      const animatedRow = { ...row, remainingFormatted: displayFormatted };
...
var UsageAggregator = class {
  ...
  buildRows(aggregates, quotaByProvider) {
    ...
    return ROW_ORDER.map((bucket) => {
      ...
      let displayValue = state.tokens;
      let displayFormatted = formatTokens(state.tokens);
      if (hasRemainingQuota && quota.remaining !== null) {
        displayFormatted += `  (${quota.remaining}, ${quota.percentage}%)`;
      }
      return {
        bucket,
        ...
        remainingFormatted: displayFormatted,
