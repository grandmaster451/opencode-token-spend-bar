  ...
  it('keeps provider rows separate across a mid-session switch and preserves state after restart', async () => {
    ...
    firstHarness.emitMessageUpdated(
      createAssistantMessage({
        ...
        providerID: 'minimax',
        cost: 1.5,
        tokens: { total: 1000, input: 400, output: 600, reasoning: 0, cache: { read: 0, write: 0 } },
    ...
    firstHarness.emitMessageUpdated(
      createAssistantMessage({
        ...
        providerID: 'openai',
        cost: 20,
        tokens: { total: 2500, input: 1200, output: 1300, reasoning: 0, cache: { read: 0, write: 0 } },
    ...
    expect(firstHarness.getRenderRequestCount()).toBe(2);
    ...
    firstHarness.emitMessageUpdated(
      createAssistantMessage({
        ...
        providerID: 'minimax',
        cost: 9,
        tokens: { total: 9000, input: 4500, output: 4500, reasoning: 0, cache: { read: 0, write: 0 } },
    ...
    expect(historyScanner.scanCurrentMonthHistory).toHaveBeenCalledTimes(1);
